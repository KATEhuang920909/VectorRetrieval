...
class HNSW(object):
    ...
    def add(self, elem, ef=None):
        ...
        if point is not None:  # the HNSW is not empty, we have an entry point ，point 是某一层的入口
            dist = distance(elem, data[point]) # 初始化距离
            # for all levels in which we dont have to insert elem,
            # we search for the closest neighbor
            for layer in reversed(graphs[level:]): # 从高往低遍历
